...
public class ProfileServlet extends HttpServlet {
	...
	protected void doGet(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (sessionUser != null && userId != sessionUser.getUserId()) {
			isFriend = userDAO.areFriends(sessionUser.getUserId(), userId);
		}
		...
		List<User> pendingRequests = userDAO.getPendingFriendRequests(userId);
		...
		if (searchQuery != null && !searchQuery.trim().isEmpty()) {
			List<User> searchResults = userDAO.searchUsersByUsername(searchQuery.trim());
	...
	protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
		...
		if (bio != null && !bio.trim().isEmpty()) {
			userDAO.updateUserBio(sessionUser.getUserId(), bio);
			sessionUser.setBio(bio);
			req.getSession().setAttribute("user", sessionUser);
		}

		resp.sendRedirect("ProfileServlet?userId=" + sessionUser.getUserId());
	}
